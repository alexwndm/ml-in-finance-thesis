...
# Advances in Financial Machine Learning, Marcos Lopez de Prado, pp.240-242
...
def correlDist(corr):
    ...
    n = dist.shape[0]
    new_dist = pd.DataFrame(0, index=np.arange(n), columns=np.arange(n))
    for i in np.arange(n):
        for j in np.arange(i + 1, n):
            eucl = euclidean(dist.iloc[:, i], dist.iloc[:, j])
            new_dist.iloc[i, j] = eucl
            new_dist.iloc[j, i] = eucl
    return new_dist
#------------------------------------------------------------------------------
...
def main():
    ...
    # df = pd.read_csv("C:/Users/Danie/Desktop/MA_local/data/TestDatasetAll.csv", index_col="Date", parse_dates=True)
    # # df = df[df.IndexMember >= 1]
    # x = pd.pivot(index="Date",
    #                    columns="AssetSymbol",
    #                    values="Return1",
    #                    data=df[["AssetSymbol", "Return1"]].reset_index())
    # x.to_csv("C:/Users/Danie/Desktop/MA_local/data/returns.csv")
